// Blast Arena Advance is licensed under the Affero General Public License v3.
// (c) Mathew Carr 2006
...
#define NDS_CHECKSUM 0xBAAE1880
#define GBA_CHECKSUM 0xBAAE187F

u32 is_this_nds;

u32 getChecksum(void) {
   u32 result;
...
void makeDefaultState(void) {
   ...
   M_H_E(bestscoretable, 0,  50,  75,  K,  a,  m,  i,  n,  e,  k,  o);
   M_H_E(bestscoretable, 1,  45,  60,  C,  h,  e,  s,  s,  e,  c, sp);
   M_H_E(bestscoretable, 2,  40,  50,  T,  o,  m,  o,  C,  h,  a,  n);
   M_H_E(bestscoretable, 3,  35,  45,  U,  r,  i,  k,  o, sp, sp, sp);
   M_H_E(bestscoretable, 4,  30,  40,  N,  a,  g,  i, sp, sp, sp, sp);
   M_H_E(bestscoretable, 5,  25,  35,  V,  a,  s,  h, pe,  T, pe,  S);
   M_H_E(bestscoretable, 6,  20,  30,  J, pe,  F, pe, sp,  M,  a,  n);
   M_H_E(bestscoretable, 7,  15,  25,  L,  i,  s,  t,  e,  r, sp, sp);
   M_H_E(bestscoretable, 8,  10,  20,  L,  a,  r,  r,  y, pe, sp,  I);
   M_H_E(bestscoretable, 9,   5,  15,  M,  a,  r,  i,  o, sp, sp, sp);

   M_H_E(besttimestable, 0,  30,  90,  S,  a,  k,  a,  k,  i, sp, sp);
   M_H_E(besttimestable, 1,  25,  75,  W,  o,  l,  f, sp, sp, sp, sp);
   M_H_E(besttimestable, 2,  20,  60,  C,  h,  i,  y,  o, sp, sp, sp);
   M_H_E(besttimestable, 3,  15,  50,  Y,  o,  s,  h,  i, sp, sp, sp);
   M_H_E(besttimestable, 4,  10,  40,  B,  l,  i,  n,  x, sp, sp, sp);
   M_H_E(besttimestable, 5,   8,  30,  C,  o,  l,  i,  n, pe, sp,  T);
   M_H_E(besttimestable, 6,   6,  25,  J,  a,  m,  e,  s, sp, sp, sp);
   M_H_E(besttimestable, 7,   4,  20,  M,  u,  l,  d,  e,  r, sp, sp);
   M_H_E(besttimestable, 8,   2,  15,  R,  a,  i,  d,  e,  n, sp, sp);
   M_H_E(besttimestable, 9,   0,  10,  A,  s,  h,  l,  e,  y, pe,  K);
...
#define CHEAT_JUST_CANT_SAVE   bit0       // Activated by any cheat. - You can't save to SRAM!
#define CHEAT_GRAVITY_ON       bit1       // heavybox                - The box is continually going down.  0
#define CHEAT_FRICTIONLESS_ON  bit2       // slippy                  - The box experiences no friction.    1
#define CHEAT_REVERSE_ON       bit3       // ms arena                - The controls are reversed.          2
#define CHEAT_SUPERSLOW_ON     bit4       // b. allen                - The box slower than molasses.       3
#define CHEAT_ALTERNATE_ON     bit5       // weirdly                 - The box is weirdly controlled.      4
#define CHEAT_MULTIPLELIVES_ON bit6       // retries                 - QTE yourself back to life.          5
#define CHEAT_INVERTCOLOURS_ON bit7       // sketchy                 - The game is black on white.         6
                                          // paulmode                - The hiscores are really harsh.      7

#define NO_MAX_CHEATS          7

vu32 cheatmodes   = 0;//CHEAT_SUPERSLOW_ON;// CHEAT_GRAVITY_ON | CHEAT_FRICTIONLESS_ON;
vu32 lastunlocked = 0;
bool randomedcheat = false;
...
#define HBLANK_REVELWIPE       6
#define HBLANK_GOSREVEAL       7
...
#define HBLANK_SRAM1_CORONA 32
         if (VCOUNTER > (hfadeline)) {
            ...
            REG_BLDMOD = bit2 | bit1 | bit7;
            REG_COLEY  = HBLANK_SRAM1_FADEFORMULA1;
         } else  {
            REG_BLDMOD = 0;
         }
   } else if (HBlanksituation == HBLANK_REVELWIPE) {
#define HBLANK_RE1_HFWIDTH 6
#define HBLANK_RE1_FADEFORMULA1 ((VCOUNTER - (hfadeline-HBLANK_RE1_CORONA-HBLANK_RE1_HFWIDTH))>>1)
#define HBLANK_RE1_CORONA 32
         if (VCOUNTER > (hfadeline)) {
            REG_BLDMOD = bit4 | bit2 | bit1 | bit6 | bit8;  // TOTAL TRANSPARENCY FROM b1 TO B0
            REG_COLEV  = 16 << 8;
         } else if (VCOUNTER > (hfadeline-HBLANK_RE1_HFWIDTH)) {
            REG_BLDMOD = bit4 | bit2 | bit1 | bit7;
            REG_COLEY  = 16;
         } else if (VCOUNTER > (hfadeline - HBLANK_RE1_CORONA-HBLANK_RE1_HFWIDTH)) {
            REG_BLDMOD = bit4 | bit2 | bit1 | bit7;
            REG_COLEY  = HBLANK_RE1_FADEFORMULA1;
         } else  {
            REG_BLDMOD = 0;
         }
...
#define DABI 6
   ...
   if (bitset(cheatmodes, CHEAT_INVERTCOLOURS_ON)) {
      for (u8 i=0; i<255; i++) {
         pal_obj_mem[i] ^= 0x7fff;
         pal_bg_mem[i] ^= 0x7fff;
      }                     
            
      bitmappos(0, 0) = 0x7fff;
      bitmappos(1, 0) = 0x7fff; 
      cpy((void *)MEM_VRAM,(void *)0x06000004,19199);
      ...
      if (GameStatus != GAME_WHITE) {
         if (bitset(sfxflags, SCOPE_ON)) {
            if ((GameStatus != GAME_BLINX) && (GameStatus != GAME_BLINXH)) {
               ...
               for (u32 soundposition = 0; soundposition < 240; soundposition++) {
                  bitmappos(soundposition, previouswave[soundposition]) = pal_bg_mem[0];
                  if (is_this_nds) {
                     if (soundposition == 0) {
                        for (u32 xlines = 0; xlines < 240; xlines++) {
                           bitmappos(xlines,   0) = 10570;
                           bitmappos(xlines, 159) = 10570;
                        }
                        for (u32 ylines = 0; ylines < 160; ylines++) {
                           bitmappos(  0, ylines) = 10570;
                           bitmappos(239, ylines) = 10570;
                        }
                     }
                  }
                  s16 ploty = 80;
...
#define gos_game_blast oi_buffer[17].attr0 = oi_buffer[17].attr1 = oi_buffer[17].attr2 = 0;  \
                  ...
                  if ((!bitset(keydowngos, KEY_L)) && (!bitset(keydowngos, KEY_R))) {
                     if ((_key_down(KEY_L)) && (_key_down(KEY_R)) && (key_hit(KEY_SELECT)) && (remaininglives != 0)) {
                        // Initiate.
                        remaininglives--;
...
void SetUpMattLogo() {
   cpy ((void *)&mattlogoData[0], (void *)&tl_mem[0][0], 9600);
   cpy ((void *) &mattlogoPal[0], (void *)&pal_bg_mem[0], 128);

   if (is_this_nds) {
      cpy ((void *)&detectedData[0], (void *)0x06009600, 480);
   }
}
...
void DoMattLogo() {
   ...
   BGINFO back1;
   bg_init(&back1, 1, 1, 30, BG_REG_1X1 | BG_PAL256);
   TEGEL *tegelset1 = back1.map;
   bg_set_pos(&back1, -162, -5);
   bg_update(&back1);

   for (u32 curtile = 0; curtile < 20; curtile++) {
      settegelrun256(tegelset0, 0, curtile, curtile*30, 30);
   }

   if (is_this_nds) {                        
      for (u32 curtilex = 0; curtilex < 32; curtilex++) {
         for (u32 curtiley = 0; curtiley < 20; curtiley++) {
            settegel(tegelset1, curtilex, curtiley, 374);
         }
      }
      for (u32 curtile = 0; curtile < 3; curtile++) {
         //settegelrun256(tegellingset, xpos, ypos, index, runcount)
         settegelrun256(tegelset1, 0, curtile, 344 + curtile*10, 10);
      }
   }

   REG_COLEY   = 16;
   REG_BLDMOD  = bit7 | bit0 | bit1;
   REG_DISPCNT = 0 | VID_BG0;
   ...
   vu16 gbaxtimeinVblanks = 0;
   
   u32 disclaimer_show = 0;

#define GBAXFADEINENDS 60
#define GBAXSTAYLENGTH 128
#define GBAXFADEOUTBEGINS (GBAXFADEINENDS+GBAXSTAYLENGTH)
#define GBAXFADEOUTENDS (GBAXFADEOUTBEGINS+32)

   while (gbaxtimeinVblanks < GBAXFADEOUTENDS) {
      VSync();
      dont_optimize();
      if (_key_down(KEY_SELECT)) {
         disclaimer_show = 1;
      }

      if (gbaxtimeinVblanks < GBAXFADEINENDS) {
         REG_COLEY  = 16 - (gbaxtimeinVblanks >> 2);
      } else if (gbaxtimeinVblanks == GBAXFADEINENDS) {
         REG_COLEY  = 0;
         REG_BLDMOD = 0;
      } else if (gbaxtimeinVblanks >= GBAXFADEOUTBEGINS) {
         REG_COLEY = ((gbaxtimeinVblanks-GBAXFADEOUTBEGINS) >> 1);
      }
      if (gbaxtimeinVblanks == GBAXFADEOUTBEGINS) {
         REG_BLDMOD  = bit7 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
      }
      gbaxtimeinVblanks += 1;
   }
...
void SetUpDisclaimer() {
   cpy ((void *)&disclaimerData[0], (void *)&tl_mem[0][0], 9600);
   cpy ((void *) &disclaimerPal[0], (void *)&pal_bg_mem[0], 128);
}

void DoDisclaimer() {
   BGINFO back0;
   bg_init(&back0, 0, 0, 31, BG_REG_1X1 | BG_PAL256);
   TEGEL *tegelset0 = back0.map;
   bg_set_pos(&back0, 0, 0);
   bg_update(&back0);
...
#define DISCFADEINENDS 60
#define DISCSTAYLENGTH 158
#define DISCFADEOUTBEGINS (DISCFADEINENDS+DISCSTAYLENGTH)
#define DISCFADEOUTENDS (DISCFADEOUTBEGINS+32)

   while (gbaxtimeinVblanks < DISCFADEOUTENDS) {
      VSync();
      dont_optimize();
      if (_key_down(KEY_SELECT)) {
         disclaimer_show = 1;
      ...
      if (gbaxtimeinVblanks < DISCFADEINENDS) {
         REG_COLEY  = 16 - (gbaxtimeinVblanks >> 2);
      } else if (gbaxtimeinVblanks == DISCFADEINENDS) {
         REG_COLEY  = 0;
         REG_BLDMOD = 0;
      } else if (gbaxtimeinVblanks >= DISCFADEOUTBEGINS) {
         REG_COLEY = ((gbaxtimeinVblanks-DISCFADEOUTBEGINS) >> 1);
      }
      if (gbaxtimeinVblanks == DISCFADEOUTBEGINS) {
         REG_BLDMOD  = bit7 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
      }
      gbaxtimeinVblanks += 1;
   }
   ...
   while (1) {
      ...
      if (titlescreentimeinVblanks < 128) {
         ...
         if (tscreenstat != BAT_NOTHING) {
            ...
            if (_key_down(KEY_START)) {
               AAS_SFX_Enhanced_Play(0, 64, 22000, AAS_DATA_SFX_START_selecrtan, AAS_DATA_SFX_END_selecrtan, AAS_NULL);//AAS_DATA_SFX_START_explos);
...
void SetUpMainMenu() {
   ...
   if (cheatmodes != 0) {
      for (u8 palettereplace = 0; palettereplace < 28; palettereplace++) {
         pal_bg_mem[2+palettereplace] = _xRGB15(palettereplace+3, (palettereplace>>2)+1, (palettereplace>>2)+1);
      }

      for (u8 palettereplace = 0; palettereplace < 32; palettereplace++) {
         pal_bg_mem[29+palettereplace] = _xRGB15(31, (palettereplace>>1), (palettereplace>>1));
      }

      for (u8 palettereplace = 0; palettereplace < 19; palettereplace++) {
         pal_bg_mem[61+palettereplace] = _xRGB15(26, (palettereplace>>2), (palettereplace>>2));
      }

      *(u16 *)0x050000A0 = _RGB15(23,5,5);
   } */

   pal_obj_mem[33] = pal_bg_mem[2];


   cpy((void *)velvetData,  (void *)&tl_mem[1][0], 4096);
   cpy((void *)velvetPal ,  (void *)&pal_bg_mem[82], 48);
}
...
int MainMenu() {
   ...
   BGINFO back2;
   bg_init(&back2, 2, 1, 30, BG_AFF_WRAP | 3);
   TEGEL *tegelset2 = back2.map;
   ...
   u16 *tegelpt16 = (u16 *)temptegel;
   u8  *tegelpt8  = (u8  *)temptegel;

   for (u32 tl = 0; tl < 256; tl++) {
      tegelpt8[tl] = tl;
   }
   for (u16 tl = 0; tl < 128; tl++) {
      tegelset2[tl] = tegelpt16[tl];
   }
...
#define SWITCH_OFF_ON_MENU_MUSIC if (bitset(sfxflags, MUSIC_ON)) {         \
   ...
   while ( ((MENUSTATUS == MENU_STATUS_NONE)
      ...
      if (MENUSTATUS == MENU_STATUS_CLEARTOGAME) {
         ...
         AAS_MOD_SetVolume(255 - ((menutimeinVblanks - menutimefadebase) << 4));
      ...
      if ((menutimeinVblanks > MENU_FADEENDS) && (MENUSTATUS == MENU_STATUS_NONE)) {
         ...
         if (key_hit(KEY_DOWN) && !key_hit(KEY_UP)) {
            if (cursormenu != 6) {
               cursormenu++;
               AAS_SFX_Enhanced_Play(0, 40, 24000, AAS_DATA_SFX_START_movesel, AAS_DATA_SFX_END_movesel, AAS_NULL);//AAS_DATA_SFX_START_explos);
            }
         }
         ...
         if (((REG_P1 == 0x3fe) && (key_hit(KEY_A))) || ((REG_P1 == 0x3f7) && (key_hit(KEY_START)))) {
            if (cursormenu == 0) {
               ...
               sfxflags ^= SOUND_ON;
               AAS_SFX_Enhanced_Play(0, 64, 24000, AAS_DATA_SFX_START_selection, AAS_DATA_SFX_END_selection, AAS_NULL);//AAS_DATA_SFX_START_explos);
   ...
   if (randomedcheat == false) {
      randomedcheat = true;
      lastunlocked = rng_rand(8);
   }
...
void SetUpHiscores() {
   ...
   cpy((void *)BGspritesPal , (void *)&pal_obj_mem[0], 114);
...
#define darkencolourSWITCH(pallete, palentry, amount) tempcolor = pal_##pallete##_mem[palentry];                                 \
                                                      redtemp   = (((tempcolor & 0x03e0) >>  5) - amount);                         \
                                                      greentemp = (((tempcolor & 0x03e0) >>  5) - amount);                         \
                                                      bluetemp  = (((tempcolor & 0x7c00) >> 10) - amount);                         \
                                                      pal_##pallete##_mem[palentry] = _xRGB15(((redtemp   < 0) ? 0 :   redtemp),  \
                                                                                              ((greentemp < 0) ? 0 : greentemp),  \
...
typedef struct tagPalUno {
   u16  color[256];
} PALUNO;       // this is 512 bytes big, yeah!
...
#define darkennameentrypalSWITCH(amount) for (u8 palentry = 9; palentry < 16; palentry++) {  darkencolourSWITCH(bg,     palentry, amount);   \
                                                                                             darkencolourSWITCH(bg,0x10+palentry, amount);   \
                                                                                             darkencolourSWITCH(bg,0x20+palentry, amount);}  \
                                         for (u16 palentry = 0; palentry < 208; palentry++) {darkencolourSWITCH(bg,0x30+palentry, amount);} \
                                                                                             darkencolourSWITCH(bg,0, amount);               \
                                                                                             darkencolourSWITCH(bg,16, amount);              \
                                                                                             darkencolourSWITCH(bg,32, amount);

#define nameentrybrightness2(amount) cpy((void *)speedyglowboxPal , (void *)&pal_bg_mem[0], 128);  darkennameentrypal((31-amount));
#define nameentrybrightness2SWITCH(amount) cpy((void *)speedyglowboxPal , (void *)&pal_bg_mem[0], 128);  darkennameentrypalSWITCH((31-amount));
      ...
      if (NAME_STATUS == NAME_STATUS_FADEIN) {
         ...
         if ((REG_P1 == 0x3df) && (key_hit(KEY_LEFT))) {
            if (citem == 0) {
               if (cx != 0) {
                  cx--;           
                  AAS_SFX_Enhanced_Play(0, 9, 34000, AAS_DATA_SFX_START_snakeclix, AAS_DATA_SFX_END_snakeclix, AAS_NULL);//AAS_DATA_SFX_START_explos);
...
// NAME ENTRY CHEAT COMPARISON. Can be used to implement whatever cheats I like. "LOL." ;)
               ...
               if (necc(R,E,S,E,T,sp,sp,sp)) {
                  ...
                  lastunlocked = 5;
                  randomedcheat = true;
               } else if (necc(s,k,e,t,c,h,y,sp)) {
                  replayername(E,n,a,b,l,e,d,ex);
                  cheatmodes |= CHEAT_JUST_CANT_SAVE | CHEAT_INVERTCOLOURS_ON;
                  curchar = 0;
                  charmode = NAME_I_UPPERSHIFT;
                  ne_box1(charmode, 24);
                  ne_box3(charmode, 168);
                  ...
                  lastunlocked = 6;    
                  randomedcheat = true;
               } else if (necc(s,c,r,a,m,b,l,e)) {
                  teppcpy(BLAST_ARENA_SRAM_START, (BLAST_ARENA_SRAM_START+1), 0xff);

                  REG_BLDMOD  = bit7 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
                  REG_COLEY   = 0;

                  for (u32 fading = 0; fading < 129; fading++) {
                     dont_optimize();
                     REG_COLEY   = (fading >> 3);
                     VSync();
                  }
...
void SRAMMissingWarning() {
   ...
   TEGEL *tegelset1 = back1.map;
   bg_set_pos(&back1, 0, 0+4);
   bg_update(&back1);

   BGINFO back2;
   bg_init(&back2, 2, 0, 29, BG_REG_1X1 | 2);
   TEGEL *tegelset2 = back2.map;
   bg_set_pos(&back2, 0-4, 0+4);
   bg_update(&back2);
   ...
   sram_line( 2,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_linex( 3,sp,sp, B, l, a, s, t,sp, A, r, e, n, a,sp, c, a, n, n, o, t,sp, f, i, n, d,sp, a,sp,sp);
   sram_line( 4,sp,sp, v, a, l, i, d,sp, h, i, s, c, o, r, e,sp, f, i, l, e,sp, o, n,sp, t, h, i, s,sp,sp);
   sram_line( 5,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp, c, a, r, t, r, i, d, g, e,pe,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_line( 6,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_line( 7,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_linex( 8,sp,sp,sp, I, f,sp, t, h, i, s,sp, y, o, u, r,sp, f, i, r, s, t,sp, t, i, m, e,sp,sp,sp);
   sram_linex( 9,sp,sp,sp,sp,sp, p, l, a, y, i, n, g,sp, t, h, e, n,sp, r, e, l, a, x,ex,sp,sp,sp,sp,sp);
   sram_line(10,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_line(12,sp,sp,sp,sp,sp, B, l, a, s, t,sp, A, r, e, n, a,sp, w, i, l, l,sp, n, o, w,sp,sp,sp,sp,sp);
   sram_line(13,sp,sp,sp, r, e, s, e, t,sp, t, h, e,sp, h, i, s, c, o, r, e,sp, t, a, b, l, e, s,sp,sp,sp);
   sram_linex(14,sp,sp,sp,sp,sp,sp,sp,sp,sp, a, n, d,sp, o, p, t, i, o, n, s,pe,sp,sp,sp,sp,sp,sp,sp,sp);
   sram_line(15,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
...
void SetUpCredits() {
   ...
   cpy((void *)snakeblocksPal, (void *)&pal_bg_mem[0], 3);
   ...
   cpy((void *)spritelightData, (void *)&tl_mem[4][1], 40);
   cpy((void *)spritelightPal, (void *)&pal_obj_mem[0], 16);

   cpy((void *)blastbonusData, (void *)&tl_mem[4][6], 128);

   pal_bg_mem[0] = pal_bg_mem[16];
}

#define cred_line(LI,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac,ad) settegel(tegelset1, 0,LI,((2<<12) | _CX_( a))); \
                                                                                      settegel(tegelset1, 1,LI,((2<<12) | _CX_( b))); \
                                                                                      settegel(tegelset1, 2,LI,((2<<12) | _CX_( c))); \
                                                                                      settegel(tegelset1, 3,LI,((2<<12) | _CX_( d))); \
                                                                                      settegel(tegelset1, 4,LI,((2<<12) | _CX_( e))); \
                                                                                      settegel(tegelset1, 5,LI,((2<<12) | _CX_( f))); \
                                                                                      settegel(tegelset1, 6,LI,((2<<12) | _CX_( g))); \
                                                                                      settegel(tegelset1, 7,LI,((2<<12) | _CX_( h))); \
                                                                                      settegel(tegelset1, 8,LI,((2<<12) | _CX_( i))); \
                                                                                      settegel(tegelset1, 9,LI,((2<<12) | _CX_( j))); \
                                                                                      settegel(tegelset1,10,LI,((2<<12) | _CX_( k))); \
                                                                                      settegel(tegelset1,11,LI,((2<<12) | _CX_( l))); \
                                                                                      settegel(tegelset1,12,LI,((2<<12) | _CX_( m))); \
                                                                                      settegel(tegelset1,13,LI,((2<<12) | _CX_( n))); \
                                                                                      settegel(tegelset1,14,LI,((2<<12) | _CX_( o))); \
                                                                                      settegel(tegelset1,15,LI,((2<<12) | _CX_( p))); \
                                                                                      settegel(tegelset1,16,LI,((2<<12) | _CX_( q))); \
                                                                                      settegel(tegelset1,17,LI,((2<<12) | _CX_( r))); \
                                                                                      settegel(tegelset1,18,LI,((2<<12) | _CX_( s))); \
                                                                                      settegel(tegelset1,19,LI,((2<<12) | _CX_( t))); \
                                                                                      settegel(tegelset1,20,LI,((2<<12) | _CX_( u))); \
                                                                                      settegel(tegelset1,21,LI,((2<<12) | _CX_( v))); \
                                                                                      settegel(tegelset1,22,LI,((2<<12) | _CX_( w))); \
                                                                                      settegel(tegelset1,23,LI,((2<<12) | _CX_( x))); \
                                                                                      settegel(tegelset1,24,LI,((2<<12) | _CX_( y))); \
                                                                                      settegel(tegelset1,25,LI,((2<<12) | _CX_( z))); \
                                                                                      settegel(tegelset1,26,LI,((2<<12) | _CX_(aa))); \
                                                                                      settegel(tegelset1,27,LI,((2<<12) | _CX_(ab))); \
                                                                                      settegel(tegelset1,28,LI,((2<<12) | _CX_(ac))); \
                                                                                      settegel(tegelset1,29,LI,((2<<12) | _CX_(ad)));


#define cred_gone(LI) settegel(tegelset1, 0,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 1,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 2,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 3,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 4,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 5,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 6,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 7,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 8,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1, 9,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,10,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,11,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,12,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,13,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,14,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,15,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,16,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,17,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,18,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,19,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,20,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,21,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,22,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,23,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,24,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,25,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,26,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,27,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,28,LI,((2<<12) | _CX_(cp))); \
                      settegel(tegelset1,29,LI,((2<<12) | _CX_(cp)));


void ba_panel_sprites(s32 yval, u32 bac);

u8 Credits() {

   AAS_MOD_Enhanced_Play(AAS_DATA_MOD_batitle);
   ...
   BGINFO back1;
   bg_init(&back1, 1, 3, 29, BG_REG_2X1 | 1);
   TEGEL *tegelset1 = back1.map;
   bg_set_pos(&back1, 0-5, 1+4-4);
   bg_update(&back1);
...
#define CREDITS_ARENABOUNDARYY (32 - 4)

   for (u16 xtile = 0; xtile < 1024; xtile++) {
      settegel(tegelset0, xtile, 0, 0);
      settegel(tegelset1, xtile, 0, _CX_(cp));
      settegel(tegelset1, xtile+1024, 0, _CX_(cp));
      settegel(tegelset2, xtile, 0, xtile | (1 << 12));
      settegel(tegelset3, xtile, 0, 86    | (2 << 12));
   }

   cred_line(16,cp,cp,cp,cp,cp, B, L, A, S, T,cp, A, R, E, N, A,cp, A, D, V, A, N, C, E,cp,cp,cp,cp,cp,cp);
   cred_line(18,cp,cp,cp,cp, A,cp, g, a, m, e,cp, b, y,cp, M, a, t, h, e, w,cp, C, a, r, r,cp,cp,cp,cp,cp);

   for (u16 ytile = 0; ytile < (2 + (CREDITS_ARENABOUNDARYY >> 1)); ytile++) {
      settegel(tegelset3, 0,  ytile, 80 | (2 << 12));
      settegel(tegelset3, 30, ytile, 83 | (2 << 12));
   }

   for (u16 xtile = 0; xtile < 31; xtile++) {
      settegel(tegelset3, xtile,  0, 81 | (2 << 12));
      settegel(tegelset3, xtile, (1 + (CREDITS_ARENABOUNDARYY >> 1)), 84 | (2 << 12));
   }

   for (u16 xtile = 1; xtile < 30; xtile++) {
      for (u16 ytile = 1; ytile < (1 + (CREDITS_ARENABOUNDARYY >> 1)); ytile++) {
         settegel(tegelset3, xtile,  ytile, 79);
      }
   }


   settegel(tegelset3,  0,  0,               82 | (2 << 12));
   settegel(tegelset3, 30,  0,        bitA | 82 | (2 << 12));
   settegel(tegelset3,  0, (1 + (CREDITS_ARENABOUNDARYY >> 1)), bitB |        82 | (2 << 12));
   settegel(tegelset3, 30, (1 + (CREDITS_ARENABOUNDARYY >> 1)),               85 | (2 << 12));
...
#define SNAKE_ORIGINAL_Y 14
   ...
   update_snake_tile((SNAKE_ORIGINAL_X - 0), SNAKE_ORIGINAL_Y);
   update_snake_tile((SNAKE_ORIGINAL_X - 1), SNAKE_ORIGINAL_Y);
   update_snake_tile((SNAKE_ORIGINAL_X - 2), SNAKE_ORIGINAL_Y);
   update_snake_tile((SNAKE_ORIGINAL_X - 3), SNAKE_ORIGINAL_Y);
   update_snake_tile((SNAKE_ORIGINAL_X - 4), SNAKE_ORIGINAL_Y);
   update_snake_tile((SNAKE_ORIGINAL_X - 5), SNAKE_ORIGINAL_Y);
...
#define CREDITS_DEADW         8

#define CREDSTAT_INTERACTIVE_CONDITION  ((CREDSTAT == CREDITS_GAMEPLAY) || (CREDSTAT == CREDITS_FADEON) || (CREDSTAT == CREDITS_FADEINLOSER))
   ...
   while ((CREDSTAT != CREDITS_DEAD) && (CREDSTAT != CREDITS_DEADW)) {
      bgxo += 2;
      bgyo += 1;
      if (bgyo > (256 << 4)) {
         bgyo -= (256 << 4);
...
#define BAP_FULLYDOWN  13
#define BAP_DONEDOWN  136
#define BAP_DONEALL   184

      if (ba_panel_frame < BAP_DONEALL) {
         if (ba_panel_frame < BAP_FULLYDOWN) {
            ba_panel_sprites(ba_panel_frame * 2 - 16, bacollected);
         } else if (ba_panel_frame > BAP_DONEDOWN) {
            ba_panel_sprites(8-((ba_panel_frame - BAP_DONEDOWN)>>1), bacollected);
         }
         ba_panel_frame++;
      }
...
#define CF_1_STARTOUT  300
#define CF_1_DONEOUT   385
#define CF_2_DONEIN    470
#define CF_2_LENGTH    (CF_2_DONEIN - CF_1_DONEOUT)

#define CF_2_STARTOUT  770
#define CF_2_DONEOUT   855
#define CF_3_DONEIN    940
#define CF_3_LENGTH    (CF_3_DONEIN - CF_2_DONEOUT)

#define CF_3_STARTOUT 1240
#define CF_3_DONEOUT  1325
#define CF_4_DONEIN   1410
#define CF_4_LENGTH    (CF_4_DONEIN - CF_3_DONEOUT)

#define CF_4_STARTOUT 1710
#define CF_4_DONEOUT  1795
#define CF_5_DONEIN   1880
#define CF_5_LENGTH    (CF_5_DONEIN - CF_4_DONEOUT)

#define CF_5_STARTOUT 2180
#define CF_5_DONEOUT  2265
#define CF_6_DONEIN   2350
#define CF_6_LENGTH    (CF_6_DONEIN - CF_5_DONEOUT)

#define CF_6_STARTOUT 2650
#define CF_6_DONEOUT  2735
#define CF_7_DONEIN   2820
#define CF_7_LENGTH    (CF_7_DONEIN - CF_6_DONEOUT)

#define CF_7_STARTOUT 3120
#define CF_7_DONEOUT  3205
#define CF_8_DONEIN   3290
#define CF_8_LENGTH    (CF_8_DONEIN - CF_7_DONEOUT)

#define CF_8_STARTOUT 3590
#define CF_8_DONEOUT  3675
#define CF_9_DONEIN   3760
#define CF_9_LENGTH    (CF_9_DONEIN - CF_8_DONEOUT)

#define CF_9_STARTOUT  4060
#define CF_9_DONEOUT   4145
#define CF_10_DONEIN   4230
#define CF_10_LENGTH   (CF_10_DONEIN - CF_9_DONEOUT)

#define CF_10_STARTOUT 4530
#define CF_10_DONEOUT  4615
#define CF_11_DONEIN   4700
#define CF_11_LENGTH   (CF_11_DONEIN - CF_10_DONEOUT)

#define CF_11_STARTOUT 5000
#define CF_11_DONEOUT  5085
#define CF_12_DONEIN   5170
#define CF_12_LENGTH   (CF_12_DONEIN - CF_11_DONEOUT)

#define CF_12_STARTOUT 5470
#define CF_12_DONEOUT  5555
#define CF_13_DONEIN   5640
#define CF_13_LENGTH   (CF_13_DONEIN - CF_12_DONEOUT)

#define CF_13_STARTOUT 5940
#define CF_13_DONEOUT  6025
#define CF_14_DONEIN   6110
#define CF_14_LENGTH   (CF_14_DONEIN - CF_13_DONEOUT)

#define CF_14_STARTOUT 6410
#define CF_14_DONEOUT  6495
#define CF_15_DONEIN   6580
#define CF_15_LENGTH   (CF_15_DONEIN - CF_14_DONEOUT)

#define CF_15_STARTOUT 6880
#define CF_15_DONEOUT  6965
#define CF_16_DONEIN   7050
#define CF_16_LENGTH   (CF_16_DONEIN - CF_15_DONEOUT)

#define CF_16_STARTOUT 7350
#define CF_16_DONEOUT  7435
#define CF_17_DONEIN   7520
#define CF_17_LENGTH   (CF_17_DONEIN - CF_16_DONEOUT)

#define CF_17_STARTOUT 7820
#define CF_17_DONEOUT  7905
#define CF_18_DONEIN   7990
#define CF_18_LENGTH   (CF_18_DONEIN - CF_17_DONEOUT)
...
// ------------------------ CREDITS MANAGER! ---------------------------/
      ...
      if (!(cf < CF_1_STARTOUT) && (cf < CF_1_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_1_STARTOUT)*(cf - CF_1_STARTOUT))>>5)-5, 1+4-4);
         bg_update(&back1);
         

      } else if (cf == CF_1_DONEOUT) {
         cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp, B, a, s, e, d,cp, o, n,cp, B, L, A, S, T,cp, A, R, E, N, A,cp, b, y,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp, E, d, w, a, r, d,cp, F,pe,cp, B, i, d, d, u, l, p, h,cp,cp,cp,cp,cp,cp);
      } else if (!(cf < CF_1_DONEOUT) && (cf < CF_2_DONEIN)) {
#define CF_2_FRAME (cf - CF_1_DONEOUT)
         bg_set_pos(&back1, 0-5-(((CF_2_FRAME-CF_2_LENGTH)*(CF_2_FRAME-CF_2_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_2_STARTOUT) && (cf < CF_2_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_2_STARTOUT)*(cf - CF_2_STARTOUT))>>5)-5, 1+4);
         bg_update(&back1);
         

      } else if (cf == CF_2_DONEOUT) {
         //cred_gone(17);
         //cred_gone(18);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp, N, a, m, e,cp, E, n, t, r, y,cp, B, a, c, k, g, r, o, u, n, d,cp, b, y,cp, a, n, d,cp);
         cred_line(18,cp,cp, A, M, A, Z, I, N, G,mi, Y, A, Y,mi, S, U, P, E, R,cp, K, u, d, o, s,cp, t, o,cp,cp);
         cred_line(19,cp,cp,cp,cp,cp,cp,cp,cp, J, o, n, a, t, h, o, n,cp, C, a, r, r,cp,cp,cp,cp,cp,cp,cp,cp,cp);
      } else if (!(cf < CF_2_DONEOUT) && (cf < CF_3_DONEIN)) {
#define CF_3_FRAME (cf - CF_2_DONEOUT)
         bg_set_pos(&back1, 0-0-(((CF_3_FRAME-CF_3_LENGTH)*(CF_3_FRAME-CF_3_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_3_STARTOUT) && (cf < CF_3_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_3_STARTOUT)*(cf - CF_3_STARTOUT))>>5)-0, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_3_DONEOUT) {
         //cred_gone(17);
         //cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp, S, n, a, k, e,cp, C, r, e, d, i, t, s,cp, G, a, m, e,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp, I, n, s, p, i, r, e, d,cp, b, y,cp, W, i, l, l, i, a, m,cp, C, a, r, r,cp,cp,cp);
      } else if (!(cf < CF_3_DONEOUT) && (cf < CF_4_DONEIN)) {
#define CF_4_FRAME (cf - CF_3_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_4_FRAME-CF_4_LENGTH)*(CF_4_FRAME-CF_4_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_4_STARTOUT) && (cf < CF_4_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_4_STARTOUT)*(cf - CF_4_STARTOUT))>>5)+5, 1+4);
         bg_update(&back1);


      } else if (cf == CF_4_DONEOUT) {
         //cred_gone(17);
         //cred_gone(18);
         //cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp,cp, A, p, e, x,cp, A, u, d, i, o,cp, S, y, s, t, e, m,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp, p, r, o, v, i, d, e, d,cp, b, y,cp, A, p, e, x,cp, D, e, s, i, g, n, s,cp,cp,cp);
      } else if (!(cf < CF_4_DONEOUT) && (cf < CF_5_DONEIN)) {
#define CF_5_FRAME (cf - CF_4_DONEOUT)
         bg_set_pos(&back1, 0+5-4-(((CF_5_FRAME-CF_5_LENGTH)*(CF_5_FRAME-CF_5_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_5_STARTOUT) && (cf < CF_5_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_5_STARTOUT)*(cf - CF_5_STARTOUT))>>5)+5-4, 1+4);
         bg_update(&back1);


      } else if (cf == CF_5_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         //cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp,cp,cp, I, n, g, a, m, e,cp, m, u, s, i, c,cp, b, y,cp,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp, O, l, o, f,cp,sm, B, l, a, i, z, e, r,sm,cp, G, u, s, t, a, f, s, s, o, n,cp,cp);
         cred_line(19,cp, M, u, s, i, c,cp, f, r, o, m,cp, P, i, n, b, a, l, l,cp, I, l, l, u, s, i, o, n, s,cp);
      } else if (!(cf < CF_5_DONEOUT) && (cf < CF_6_DONEIN)) {
#define CF_6_FRAME (cf - CF_5_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_6_FRAME-CF_6_LENGTH)*(CF_6_FRAME-CF_6_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_6_STARTOUT) && (cf < CF_6_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_6_STARTOUT)*(cf - CF_6_STARTOUT))>>5)+5, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_6_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp, M, e, n, u,cp, a, n, d,cp, G, a, m, e,cp, O, v, e, r,cp, M, u, s, i, c,cp, b, y,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp, R, i, s, t, o,cp, V, u, o, r, i,cp,cp,cp,cp,cp,cp,cp,cp,cp);
         cred_line(19,cp,cp,cp, M, u, s, i, c,cp, f, r, o, m,cp, S, u, p, e, r,cp, S, t, a, r, d, u, s, t,cp,cp);
      } else if (!(cf < CF_6_DONEOUT) && (cf < CF_7_DONEIN)) {
#define CF_7_FRAME (cf - CF_6_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_7_FRAME-CF_7_LENGTH)*(CF_7_FRAME-CF_7_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_7_STARTOUT) && (cf < CF_7_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_7_STARTOUT)*(cf - CF_7_STARTOUT))>>5)+5, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_7_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp,cp, C, r, e, d, i, t, s,cp, m, u, s, i, c,cp, b, y,cp,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp, M, a, r, k,cp, K, n, i, g, h, t,cp,cp,cp,cp,cp,cp,cp,cp,cp);
         cred_line(19,cp,cp,cp, M, u, s, i, c,cp, f, r, o, m,cp, O, u, t,cp, t, o,cp, L, u, n, c, h,cp,cp,cp,cp);
      } else if (!(cf < CF_7_DONEOUT) && (cf < CF_8_DONEIN)) {
#define CF_8_FRAME (cf - CF_7_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_8_FRAME-CF_8_LENGTH)*(CF_8_FRAME-CF_8_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_8_STARTOUT) && (cf < CF_8_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_8_STARTOUT)*(cf - CF_8_STARTOUT))>>5)+5, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_8_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         //cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp, S, p, e, c, i, a, l,cp, T, h, a, n, k, s,cp, t, o,cp, C, e, a, r, n,cp, f, o, r,cp);
         cred_line(18,cp,cp,cp, t, h, e,cp, i, n, v, a, l, u, a, b, l, e,cp, U, s, e, n, t, i,cp, a, n, d,cp,cp);
         cred_line(19,cp,cp,cp,cp,cp, h, i, s,cp, s, u, p, e, r, b,cp, T, O, N, C,cp, g, u, i, d, e,ex,cp,cp,cp);
      } else if (!(cf < CF_8_DONEOUT) && (cf < CF_9_DONEIN)) {
#define CF_9_FRAME (cf - CF_8_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_9_FRAME-CF_9_LENGTH)*(CF_9_FRAME-CF_9_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_9_STARTOUT) && (cf < CF_9_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_9_STARTOUT)*(cf - CF_9_STARTOUT))>>5)+5, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_9_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp, U, b, e, r,cp, g, r, e, e, t, i, n, g, s,cp, t, o,cp, T, i, m, b, o, S, q, u, a, d,cp);
         cred_line(18,cp,cp,cp, m, h, 2,cp, a, n, d,cp, a, l, l,cp, t, h, e,cp, f, o, l, k, s,cp, a, t,cp,cp,cp);
         cred_line(19,cp,cp,cp,cp,hs, u, b, e, r, d, u, c, k, s,cp, a, n, d,cp,hs, u, p, l, i, n, k,ex,cp,cp,cp);
      } else if (!(cf < CF_9_DONEOUT) && (cf < CF_10_DONEIN)) {
#define CF_10_FRAME (cf - CF_9_DONEOUT)
         bg_set_pos(&back1, 0+5-4-(((CF_10_FRAME-CF_10_LENGTH)*(CF_10_FRAME-CF_10_LENGTH))>>5), 1+4+4);
         bg_update(&back1);
      } else if (!(cf < CF_10_STARTOUT) && (cf < CF_10_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_10_STARTOUT)*(cf - CF_10_STARTOUT))>>5)+5-4, 1+4+4);
         bg_update(&back1);


      } else if (cf == CF_10_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(16,cp,cp,cp, A, l, s, o,cp, M, E, O, W,cp, t, o,cp, t, h, e,cp, p, e, e, p, s,cp, a, t,cp,cp);
         cred_line(17,cp,cp,cp,cp,cp, g, b, a, d, e, v,pe, o, r, g,cm,cp, G, B, A, X,pe, c, o, m,cm,cp,cp,cp,cp);
         cred_line(18,cp, e, m, u, h, o, l, i, c,pe, c, o, m,cp, a, n, d,cp, j, a, n, d, a, m, a, n,pe, c, o, m);
      } else if (!(cf < CF_10_DONEOUT) && (cf < CF_11_DONEIN)) {
#define CF_11_FRAME (cf - CF_10_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_11_FRAME-CF_11_LENGTH)*(CF_11_FRAME-CF_11_LENGTH))>>5), 1);
         bg_update(&back1);
      } else if (!(cf < CF_11_STARTOUT) && (cf < CF_11_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_11_STARTOUT)*(cf - CF_11_STARTOUT))>>5)+5, 1);
         bg_update(&back1);

      } else if (cf == CF_11_DONEOUT) {
         //cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(16,cp,cp, M, a, n, y,cp, t, h, a, n, k, s,cp, t, o,cp, t, h, e,cp, g, u, y, s,cp, w, h, o,cp);
         cred_line(17,cp, s, m, a, s, h, e, d,cp, o, p, e, n,cp, t, h, e, i, r,cp, G, a, m, e,cp, B, o, y, s,cm);
         cred_line(18,cp,cp,cp, m, a, k, i, n, g,cp, a, l, l,cp, t, h, i, s,cp, p, o, s, s, i, b, l, e,ex,cp,cp);
      } else if (!(cf < CF_11_DONEOUT) && (cf < CF_12_DONEIN)) {
#define CF_12_FRAME (cf - CF_11_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_12_FRAME-CF_12_LENGTH)*(CF_12_FRAME-CF_12_LENGTH))>>5), 1);
         bg_update(&back1);
      } else if (!(cf < CF_12_STARTOUT) && (cf < CF_12_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_12_STARTOUT)*(cf - CF_12_STARTOUT))>>5)+5, 1);
         bg_update(&back1);


      } else if (cf == CF_12_DONEOUT) {
         cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp, M, a, d, e,cp, u, s, i, n, g,cp, D, e, v, K, i, t, A, R, M,cp, R, 1, 1,cp, a, n, d,cp);
         cred_line(18,cp,cp,cp, a, n, d,cp, E, d, e, n,cp, K, i, r, i, n, s,cp, C, o, n, T, E, X, T,pe,cp,cp,cp);
      } else if (!(cf < CF_12_DONEOUT) && (cf < CF_13_DONEIN)) {
#define CF_13_FRAME (cf - CF_12_DONEOUT)
         bg_set_pos(&back1, 0+5-4-(((CF_13_FRAME-CF_13_LENGTH)*(CF_13_FRAME-CF_13_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_13_STARTOUT) && (cf < CF_13_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_13_STARTOUT)*(cf - CF_13_STARTOUT))>>5)+5-4, 1+4);
         bg_update(&back1);


      } else if (cf == CF_13_DONEOUT) {
         cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp,cp,cp, P, A, I, N, T,cp, S, H, O, P,cp, P, R, O,cp, 4,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp,cp, f, o, r, e, v, e, r,cm,cp, y, a, l, l,ex,cp,ca, _,ca,cp,cp,cp,cp,cp);
      } else if (!(cf < CF_13_DONEOUT) && (cf < CF_14_DONEIN)) {
#define CF_14_FRAME (cf - CF_13_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_14_FRAME-CF_14_LENGTH)*(CF_14_FRAME-CF_14_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_14_STARTOUT) && (cf < CF_14_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_14_STARTOUT)*(cf - CF_14_STARTOUT))>>5)+5, 1+4);
         bg_update(&back1);


      } else if (cf == CF_14_DONEOUT) {
         cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(17,cp,cp,cp,cp,cp,cp,cp,cp,cp, T, h, a, n, k,cp, y, o, u,cp, f, o, r,cp,cp,cp,cp,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp,cp,cp, p, l, a, y, i, n, g,cp, m, y,cp, g, a, m, e,cp,cp,cp,cp,cp,cp,cp);
      } else if (!(cf < CF_14_DONEOUT) && (cf < CF_15_DONEIN)) {
#define CF_15_FRAME (cf - CF_14_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_15_FRAME-CF_15_LENGTH)*(CF_15_FRAME-CF_15_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } else if (!(cf < CF_15_STARTOUT) && (cf < CF_15_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_15_STARTOUT)*(cf - CF_15_STARTOUT))>>5)+5, 1+4);
         bg_update(&back1);


      } else if (cf == CF_15_DONEOUT) {
         cred_gone(16);
         cred_gone(17);
         cred_gone(18);
         cred_gone(19);
         bg_set_pos(&back1, 0, 0-50);
         bg_update(&back1);
         cred_line(16,cp,cp,cp,cp,cp,cp, B, l, a, s, t,cp, A, r, e, n, a,cp, A, d, v, a, n, c, e,cp,cp,cp,cp,cp);
         cred_line(17,cp,cp,cp,cp,cp,op, C,cl,cp, 2, 0, 0, 6,cp, M, a, t, h, e, w,cp, C, a, r, r,pe,cp,cp,cp,cp);
         cred_line(18,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp,cp);
         cred_line(19,cp,cp, h, t, t, p,co,fs,fs, w, w, w,pe, m, r, d, i, c, t, i, o, n, a, r, y,pe, n, e, t,cp);
      } else if (!(cf < CF_15_DONEOUT) && (cf < CF_16_DONEIN)) {
#define CF_16_FRAME (cf - CF_15_DONEOUT)
         bg_set_pos(&back1, 0+5-(((CF_16_FRAME-CF_16_LENGTH)*(CF_16_FRAME-CF_16_LENGTH))>>5), 1+4);
         bg_update(&back1);
      } /*else if (!(cf < CF_13_STARTOUT) && (cf < CF_13_DONEOUT)) {
         bg_set_pos(&back1, (((cf - CF_13_STARTOUT)*(cf - CF_13_STARTOUT))>>5)+5, 1+4);
         bg_update(&back1);
...
// ---------------------- END CREDITS MANAGER! ---------------------------/
      ...
      if (CREDSTAT == CREDITS_FADEON) {
         ...
         if (animationframe == 32) {
            ...
            snakesdirectiono = SNAKE_RIGHT;

         }
      } else if (CREDSTAT == CREDITS_FADEINLOSER) {
         VSync();
         key_poll(); 
         REG_COLEY = 16 - (animationframe >> 1);
         animationframe++;
      ...
      } else if (CREDSTAT == CREDITS_FADEOUTQUIT) {
         VSync();
         key_poll();
         REG_COLEY = (animationframe >> 2);
         animationframe++;
         AAS_MOD_SetVolume(256-(animationframe << 2));
         if (animationframe == 64) {
            CREDSTAT = CREDITS_DEAD;
            AAS_MOD_Stop();
         }
      } else if (CREDSTAT == CREDITS_FADEOUTWINNER) {
         VSync();
         key_poll();
         REG_COLEY = (animationframe >> 3);
         animationframe++;
         if (animationframe == 128) {
            CREDSTAT = CREDITS_DEADW;
         }
      }
      ...
      if (CREDSTAT_INTERACTIVE_CONDITION) {
         ...
         if (snaketimer++ > ((framesperclick == 1) ? 1 : (framesperclick >> (_key_down(KEY_A) ? 3 : 0)))) { 
            AAS_SFX_Enhanced_Play(0, 3, 34000, AAS_DATA_SFX_START_snakeclix, AAS_DATA_SFX_END_snakeclix, AAS_NULL);//AAS_DATA_SFX_START_explos);
                  ...
                  if (snakex[0] == 0) {
                     for (u32 snakeblock = 1; !(snakeblock > snakelast); snakeblock++) {
                        if ((snakex[snakeblock] == (CREDITS_ARENABOUNDARYX - 1)) && (snakey[snakeblock] == snakey[0])) {
                           ...
                           REG_BLDMOD = bit7 | bit6 | bit5 | bit4 | bit2 | bit0;
                           REG_COLEY  = 0;
                        }
                     }
                  } else {
            ...
            if (CREDSTAT != CREDITS_FADEOUTLOSER) {
               ...
               if ((snakex[0] == objectivex) && (snakey[0] == objectivey)) {
                  ...
                  if (specialbonus) {
                     ...
                     ba_panel_frame = 0;
                     if (bacollected == 10) {
                        CREDSTAT = CREDITS_FADEOUTWINNER;
                        REG_BLDMOD = bit7 | bit6 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
                        REG_COLEY  = 0;
                        animationframe = 0;
                     }
      ...
      if (specialbonus) {
         oi_buffer[0].attr0 = (((objectivey << 2)-2+4) & 255);
         oi_buffer[0].attr1 = (((objectivex << 2)-2+4) & 511);
         oi_buffer[0].attr2 = (1 << 12) | ((cycle <  2) ? 0 :
                                           (cycle <  5) ? 1 :
                                           (cycle <  9) ? 2 :
                                           (cycle < 14) ? 4 :
                                           (cycle < 42) ? 5 :
   ...
   return ((CREDSTAT == CREDITS_DEADW) ? 1 : 0);
}

#define BA_TEXT_BASE                 10
#define BA_TEXT_1      (BA_TEXT_BASE+ 0)
#define BA_TEXT_2      (BA_TEXT_BASE+ 5)
#define BA_TEXT_3      (BA_TEXT_BASE+12+ 0*8)
#define BA_TEXT_4      (BA_TEXT_BASE+12+ 1*8)
#define BA_TEXT_5      (BA_TEXT_BASE+12+ 2*8)
#define BA_TEXT_6      (BA_TEXT_BASE+12+ 3*8 + 7)
#define BA_TEXT_7      (BA_TEXT_BASE+12+ 4*8 + 7)
#define BA_TEXT_8      (BA_TEXT_BASE+12+ 5*8 + 7)
#define BA_TEXT_9      (BA_TEXT_BASE+12+ 6*8 + 7)
#define BA_TEXT_10     (BA_TEXT_BASE+12+ 7*8 + 7)
...
#define BA_TILE_8      (18)
#define BA_TILE_9      (20)
#define BA_TILE_10     (10)

#define BA_PANEL_MACRO(N) if (bac > (N-1)) {                                                    \
                             oi_buffer[N].attr0 = bitF | (yval & 255);                          \
                             oi_buffer[N].attr1 = BA_TEXT_##N;                                  \
                             oi_buffer[N].attr2 = (3 << 10) | BA_TILE_##N;                      \
                          } else {                                                              \
                             oi_buffer[N].attr0 = oi_buffer[N].attr1 = oi_buffer[N].attr2 = 0;  \
                          }

void ba_panel_sprites(s32 yval, u32 bac) {
   /*if (bac > 0) {
      oi_buffer[1].attr0 = bitE | yval;
      oi_buffer[1].attr1 = BA_TEXT_1;
      oi_buffer[1].attr2 = (1 << 10) | 6+(2*0);
   }*/
   BA_PANEL_MACRO( 1);
   BA_PANEL_MACRO( 2);
   BA_PANEL_MACRO( 3);
...
void SetUpRevelation() {
   ...
   cpy((void *)texttilesData, (void *)&tl_mem[3][1], 728);
   cpy((void *)texttilesData, (void *)&tl_mem[4][1], 728);
...
#define reve_line(LI,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac,ad) settegel(tegelset1, 0,LI,((0<<12) | _RX_( a))); \
                                                                                      settegel(tegelset1, 1,LI,((0<<12) | _RX_( b))); \
                                                                                      settegel(tegelset1, 2,LI,((0<<12) | _RX_( c))); \
                                                                                      settegel(tegelset1, 3,LI,((0<<12) | _RX_( d))); \
                                                                                      settegel(tegelset1, 4,LI,((0<<12) | _RX_( e))); \
                                                                                      settegel(tegelset1, 5,LI,((0<<12) | _RX_( f))); \
                                                                                      settegel(tegelset1, 6,LI,((0<<12) | _RX_( g))); \
                                                                                      settegel(tegelset1, 7,LI,((0<<12) | _RX_( h))); \
                                                                                      settegel(tegelset1, 8,LI,((0<<12) | _RX_( i))); \
                                                                                      settegel(tegelset1, 9,LI,((0<<12) | _RX_( j))); \
                                                                                      settegel(tegelset1,10,LI,((0<<12) | _RX_( k))); \
                                                                                      settegel(tegelset1,11,LI,((0<<12) | _RX_( l))); \
                                                                                      settegel(tegelset1,12,LI,((0<<12) | _RX_( m))); \
                                                                                      settegel(tegelset1,13,LI,((0<<12) | _RX_( n))); \
                                                                                      settegel(tegelset1,14,LI,((0<<12) | _RX_( o))); \
                                                                                      settegel(tegelset1,15,LI,((0<<12) | _RX_( p))); \
                                                                                      settegel(tegelset1,16,LI,((0<<12) | _RX_( q))); \
                                                                                      settegel(tegelset1,17,LI,((0<<12) | _RX_( r))); \
                                                                                      settegel(tegelset1,18,LI,((0<<12) | _RX_( s))); \
                                                                                      settegel(tegelset1,19,LI,((0<<12) | _RX_( t))); \
                                                                                      settegel(tegelset1,20,LI,((0<<12) | _RX_( u))); \
                                                                                      settegel(tegelset1,21,LI,((0<<12) | _RX_( v))); \
                                                                                      settegel(tegelset1,22,LI,((0<<12) | _RX_( w))); \
                                                                                      settegel(tegelset1,23,LI,((0<<12) | _RX_( x))); \
                                                                                      settegel(tegelset1,24,LI,((0<<12) | _RX_( y))); \
                                                                                      settegel(tegelset1,25,LI,((0<<12) | _RX_( z))); \
                                                                                      settegel(tegelset1,26,LI,((0<<12) | _RX_(aa))); \
                                                                                      settegel(tegelset1,27,LI,((0<<12) | _RX_(ab))); \
                                                                                      settegel(tegelset1,28,LI,((0<<12) | _RX_(ac))); \
                                                                                      settegel(tegelset1,29,LI,((0<<12) | _RX_(ad)));

#define reve_linex(LI,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac)   settegel(tegelset2, 0,LI,((0<<12) | _RX_( a))); \
                                                                                      settegel(tegelset2, 1,LI,((0<<12) | _RX_( b))); \
                                                                                      settegel(tegelset2, 2,LI,((0<<12) | _RX_( c))); \
                                                                                      settegel(tegelset2, 3,LI,((0<<12) | _RX_( d))); \
                                                                                      settegel(tegelset2, 4,LI,((0<<12) | _RX_( e))); \
                                                                                      settegel(tegelset2, 5,LI,((0<<12) | _RX_( f))); \
                                                                                      settegel(tegelset2, 6,LI,((0<<12) | _RX_( g))); \
                                                                                      settegel(tegelset2, 7,LI,((0<<12) | _RX_( h))); \
                                                                                      settegel(tegelset2, 8,LI,((0<<12) | _RX_( i))); \
                                                                                      settegel(tegelset2, 9,LI,((0<<12) | _RX_( j))); \
                                                                                      settegel(tegelset2,10,LI,((0<<12) | _RX_( k))); \
                                                                                      settegel(tegelset2,11,LI,((0<<12) | _RX_( l))); \
                                                                                      settegel(tegelset2,12,LI,((0<<12) | _RX_( m))); \
                                                                                      settegel(tegelset2,13,LI,((0<<12) | _RX_( n))); \
                                                                                      settegel(tegelset2,14,LI,((0<<12) | _RX_( o))); \
                                                                                      settegel(tegelset2,15,LI,((0<<12) | _RX_( p))); \
                                                                                      settegel(tegelset2,16,LI,((0<<12) | _RX_( q))); \
                                                                                      settegel(tegelset2,17,LI,((0<<12) | _RX_( r))); \
                                                                                      settegel(tegelset2,18,LI,((0<<12) | _RX_( s))); \
                                                                                      settegel(tegelset2,19,LI,((0<<12) | _RX_( t))); \
                                                                                      settegel(tegelset2,20,LI,((0<<12) | _RX_( u))); \
                                                                                      settegel(tegelset2,21,LI,((0<<12) | _RX_( v))); \
                                                                                      settegel(tegelset2,22,LI,((0<<12) | _RX_( w))); \
                                                                                      settegel(tegelset2,23,LI,((0<<12) | _RX_( x))); \
                                                                                      settegel(tegelset2,24,LI,((0<<12) | _RX_( y))); \
                                                                                      settegel(tegelset2,25,LI,((0<<12) | _RX_( z))); \
                                                                                      settegel(tegelset2,26,LI,((0<<12) | _RX_(aa))); \
                                                                                      settegel(tegelset2,27,LI,((0<<12) | _RX_(ab))); \
                                                                                      settegel(tegelset2,28,LI,((0<<12) | _RX_(ac)));
                                                                                      

#define revd_line(LI,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac,ad) settegel(tegelset1, 0,LI,((2<<12) | _RX_( a))); \
                                                                                      settegel(tegelset1, 1,LI,((2<<12) | _RX_( b))); \
                                                                                      settegel(tegelset1, 2,LI,((2<<12) | _RX_( c))); \
                                                                                      settegel(tegelset1, 3,LI,((2<<12) | _RX_( d))); \
                                                                                      settegel(tegelset1, 4,LI,((2<<12) | _RX_( e))); \
                                                                                      settegel(tegelset1, 5,LI,((2<<12) | _RX_( f))); \
                                                                                      settegel(tegelset1, 6,LI,((2<<12) | _RX_( g))); \
                                                                                      settegel(tegelset1, 7,LI,((2<<12) | _RX_( h))); \
                                                                                      settegel(tegelset1, 8,LI,((2<<12) | _RX_( i))); \
                                                                                      settegel(tegelset1, 9,LI,((2<<12) | _RX_( j))); \
                                                                                      settegel(tegelset1,10,LI,((2<<12) | _RX_( k))); \
                                                                                      settegel(tegelset1,11,LI,((2<<12) | _RX_( l))); \
                                                                                      settegel(tegelset1,12,LI,((2<<12) | _RX_( m))); \
                                                                                      settegel(tegelset1,13,LI,((2<<12) | _RX_( n))); \
                                                                                      settegel(tegelset1,14,LI,((2<<12) | _RX_( o))); \
                                                                                      settegel(tegelset1,15,LI,((2<<12) | _RX_( p))); \
                                                                                      settegel(tegelset1,16,LI,((2<<12) | _RX_( q))); \
                                                                                      settegel(tegelset1,17,LI,((2<<12) | _RX_( r))); \
                                                                                      settegel(tegelset1,18,LI,((2<<12) | _RX_( s))); \
                                                                                      settegel(tegelset1,19,LI,((2<<12) | _RX_( t))); \
                                                                                      settegel(tegelset1,20,LI,((2<<12) | _RX_( u))); \
                                                                                      settegel(tegelset1,21,LI,((2<<12) | _RX_( v))); \
                                                                                      settegel(tegelset1,22,LI,((2<<12) | _RX_( w))); \
                                                                                      settegel(tegelset1,23,LI,((2<<12) | _RX_( x))); \
                                                                                      settegel(tegelset1,24,LI,((2<<12) | _RX_( y))); \
                                                                                      settegel(tegelset1,25,LI,((2<<12) | _RX_( z))); \
                                                                                      settegel(tegelset1,26,LI,((2<<12) | _RX_(aa))); \
                                                                                      settegel(tegelset1,27,LI,((2<<12) | _RX_(ab))); \
                                                                                      settegel(tegelset1,28,LI,((2<<12) | _RX_(ac))); \
                                                                                      settegel(tegelset1,29,LI,((2<<12) | _RX_(ad)));

#define revd_linex(LI,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac)   settegel(tegelset2, 0,LI,((2<<12) | _RX_( a))); \
                                                                                      settegel(tegelset2, 1,LI,((2<<12) | _RX_( b))); \
                                                                                      settegel(tegelset2, 2,LI,((2<<12) | _RX_( c))); \
                                                                                      settegel(tegelset2, 3,LI,((2<<12) | _RX_( d))); \
                                                                                      settegel(tegelset2, 4,LI,((2<<12) | _RX_( e))); \
                                                                                      settegel(tegelset2, 5,LI,((2<<12) | _RX_( f))); \
                                                                                      settegel(tegelset2, 6,LI,((2<<12) | _RX_( g))); \
                                                                                      settegel(tegelset2, 7,LI,((2<<12) | _RX_( h))); \
                                                                                      settegel(tegelset2, 8,LI,((2<<12) | _RX_( i))); \
                                                                                      settegel(tegelset2, 9,LI,((2<<12) | _RX_( j))); \
                                                                                      settegel(tegelset2,10,LI,((2<<12) | _RX_( k))); \
                                                                                      settegel(tegelset2,11,LI,((2<<12) | _RX_( l))); \
                                                                                      settegel(tegelset2,12,LI,((2<<12) | _RX_( m))); \
                                                                                      settegel(tegelset2,13,LI,((2<<12) | _RX_( n))); \
                                                                                      settegel(tegelset2,14,LI,((2<<12) | _RX_( o))); \
                                                                                      settegel(tegelset2,15,LI,((2<<12) | _RX_( p))); \
                                                                                      settegel(tegelset2,16,LI,((2<<12) | _RX_( q))); \
                                                                                      settegel(tegelset2,17,LI,((2<<12) | _RX_( r))); \
                                                                                      settegel(tegelset2,18,LI,((2<<12) | _RX_( s))); \
                                                                                      settegel(tegelset2,19,LI,((2<<12) | _RX_( t))); \
                                                                                      settegel(tegelset2,20,LI,((2<<12) | _RX_( u))); \
                                                                                      settegel(tegelset2,21,LI,((2<<12) | _RX_( v))); \
                                                                                      settegel(tegelset2,22,LI,((2<<12) | _RX_( w))); \
                                                                                      settegel(tegelset2,23,LI,((2<<12) | _RX_( x))); \
                                                                                      settegel(tegelset2,24,LI,((2<<12) | _RX_( y))); \
                                                                                      settegel(tegelset2,25,LI,((2<<12) | _RX_( z))); \
                                                                                      settegel(tegelset2,26,LI,((2<<12) | _RX_(aa))); \
                                                                                      settegel(tegelset2,27,LI,((2<<12) | _RX_(ab))); \
                                                                                      settegel(tegelset2,28,LI,((2<<12) | _RX_(ac)));

void Revelation() {

   if (lastunlocked == NO_MAX_CHEATS) {
      lastunlocked = 0;
   } else {
      lastunlocked++;
   }
   ...
   BGINFO back1;
   bg_init(&back1, 1, 3, 30, BG_REG_1X1 | 2);
   TEGEL *tegelset1 = back1.map;
   bg_set_pos(&back1, 0, 0);
   bg_update(&back1);

   BGINFO back2;
   bg_init(&back2, 2, 3, 29, BG_REG_1X1 | 2);
   TEGEL *tegelset2 = back2.map;
   ...
   for (u8 curpal = 0; curpal < 22; curpal++) {
      nameentrybrightness2SWITCH((31-curpal));
      for (u16 curcol = 0; curcol < 256; curcol++) {
         tpal[curpal].color[curcol] = pal_bg_mem[curcol];
      }
      tpal[curpal].color[0] = 0;
   }
   ...
   #define RE_STATUS_FADEIN        0
   #define RE_STATUS_APPEAR        1
   #define RE_STATUS_STEADY        2
   #define RE_STATUS_DISAPPEAR     5
   #define RE_STATUS_FADEOUT       6
   #define RE_STATUS_DEAD          7

   u8 RE_STATUS = RE_STATUS_FADEIN;
   u32 animationframe = 0;  

   reve_line( 1,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_line( 2,sp,sp,sp,sp,sp,sp, E, n, t, e, r,sp, y, o, u, r,sp, n, a, m, e,sp, a, s,sp,sp,sp,sp,sp,sp);
   reve_line( 3,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_line( 4,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_line( 5,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_line( 6,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_linex( 7,sp,sp, H, o, l, d,sp, L,pl, R,sp, a, n, d,sp, p, r, e, s, s,sp, S, e, l, e, c, t,sp,sp);
   reve_line( 8,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   reve_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);

#define REVEAL_NAMEX 56
#define REVEAL_NAMEY 31

#define revealsprite(NO, TE)   oi_buffer[NO].attr0 = bit9 | bit8 | REVEAL_NAMEY;     \
                               oi_buffer[NO].attr1 = (NO<<9) | REVEAL_NAMEX+16*NO;   \
                               oi_buffer[NO].attr2 = (3 << 12) | ((_RX_(TE)));     \
                               oa_buffer[NO].pa = oa_buffer[NO].pd = (1<<8)>>1;      \
                               oa_buffer[NO].pb = oa_buffer[NO].pc = 0;

#define revealspritex(NO, TE)   oi_buffer[NO].attr0 = bit9 | bit8 | REVEAL_NAMEY;     \
                                oi_buffer[NO].attr1 = (NO<<9) | 8 + REVEAL_NAMEX+16*NO;   \
                                oi_buffer[NO].attr2 = (3 << 12) | ((_RX_(TE)));     \
                                oa_buffer[NO].pa = oa_buffer[NO].pd = (1<<8)>>1;      \
                                oa_buffer[NO].pb = oa_buffer[NO].pc = 0;


#define revealcode(a,b,c,d,e,f,g,h) revealsprite(0, a); \
                                    revealsprite(1, b); \
                                    revealsprite(2, c); \
                                    revealsprite(3, d); \
                                    revealsprite(4, e); \
...
#define revealcodex(a,b,c,d,e,f,g)  revealspritex(0, a); \
                                    revealspritex(1, b); \
                                    revealspritex(2, c); \
                                    revealspritex(3, d); \
                                    revealspritex(4, e); \
   ...
   if (lastunlocked == 0) {
      revealcode( h, e, a, v, y, b, o, x);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(10, G, e, t, t, i, n, g,sp, t, h, a, t,sp, s, i, n, k, i, n, g,sp, f, e, e, l, i, n, g,qu);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(12,sp,sp, E, n, a, b, l, e,sp, g, r, a, v, i, t, y,sp, f, o, r,sp, t, h, e,sp, b, o, x,sp,sp);
      revd_linex(13,sp,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp, t, h, i, s,sp, c, h, e, a, t,ex,sp,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(15,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 1) {
      revealcode(sp, s, l, i, p, p, y,sp);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(10,sp,sp,sp,sp,sp,sp, S, l, i, p,sp, s, l, i, d, i, n, g,sp, a, w, a, y,ex,sp,sp,sp,sp,sp,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(12,sp,sp, D, i, s, a, b, l, e,sp, f, r, i, c, t, i, o, n,sp, e, n, t, i, r, e, l, y,sp,sp);
      revd_linex(13,sp,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp, t, h, i, s,sp, c, h, e, a, t,ex,sp,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 2) {
      revealcode( m, s,sp, a, r, e, n, a);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(10,sp,sp, W, h, e, n,sp, f, o, r, w, a, r, d, s,sp, g, e, t, s,sp, o, l, d,pe,pe,pe,sp,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(12,sp,sp,sp,sp,sp, R, e, v, e, r, s, e,sp, t, h, e,sp, c, o, n, t, r, o, l, s,sp,sp,sp,sp,sp);
      revd_linex(13,sp,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp, t, h, i, s,sp, c, h, e, a, t,ex,sp,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 3) {
      revealcode( b,pe,sp, a, l, l, e, n);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(10,sp, I, n,sp, t, h, e,sp, a, s, p, i, c,sp, o, f,sp, t, h, e,sp, w, o, r, l, d,pe,pe,pe,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(12,sp, I, n, c, r, e, a, s, e,sp, t, h, e,sp, d, i, f, f, i, c, u, l, t, y,sp, a, n, d,sp);
      revd_linex(13, s, l, o, w,sp, d, o, w, n,sp, t, h, e,sp, b, o, x,sp, t, o,sp, a,sp, s, n, a, i, l, s);
      revd_linex(14, p, a, c, e,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp, t, h, i, s,sp, c, h, e, a, t);
   } else if (lastunlocked == 4) {
      revealcodex( w, e, i, r, d, l, y);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp, S, i, c, k,sp, o, f,sp, a, l, l,sp, t, h, a, t,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(10,sp,sp,sp,sp,sp,sp,sp, s, w, i, r, l, i, n, g,sp, a, b, o, u, t,qu,sp,sp,sp,sp,sp,sp,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(12,sp,sp, G, i, v, e,sp, t, h, e,sp, a, l, t, e, n, a, t, e,sp, c, o, n, t, r, o, l, s,sp,sp);
      revd_line(13,sp,sp,sp,sp,sp, a,sp, s, h, o, t,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp,sp,sp,sp,sp);
      revd_linex(14,sp,sp,sp,sp,sp,sp,sp,sp,sp, t, h, i, s,sp, c, h, e, a, t,ex,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 5) {
      revealcodex( r, e, t, r, i, e, s);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(10, W, a, n, t,sp, t, o,sp, g, i, v, e,sp, i, t,sp, a, n, o, t, h, e, r,sp, s, h, o, t,qu);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(12,sp,sp, H, o, l, d,sp, L,pl, R,sp, a, n, d,sp, t, a, p,sp, S, e, l, e, c, t,sp, o, n,sp,sp);
      revd_line(13,sp,sp,sp, t, h, e,sp, G, a, m, e,sp, O, v, e, r,sp, s, c, r, e, e, n,sp, f, o, r,sp,sp,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp, a, n, o, t, h, e, r,sp, c, h, a, n, c, e,pe,pe,pe,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 6) {
      revealcodex( s, k, e, t, c, h, y);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(10,sp,sp,sp,sp,sp,sp, B, l, a, c, k,sp, o, n,sp, w, h, i, t, e,pe,pe,pe,sp,sp,sp,sp,sp,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(12,sp,sp,sp,sp,sp, I, n, v, e, r, t,sp, t, h, e,sp, g, r, a, p, h, i, c, s,sp,sp,sp,sp,sp);
      revd_linex(13,sp,sp, b, y,sp, a, c, t, i, v, a, t, i, n, g,sp, t, h, i, s,sp, c, h, e, a, t,ex,sp,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   } else if (lastunlocked == 7) {
      revealcode( p, a, u, l, m, o, d, e);
      revd_line( 9,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_line(10,sp,sp,sp,sp,sp,sp, F, a, n, c, y,sp, a,sp, c, h, a, l, l, e, n, g, e,qu,sp,sp,sp,sp,sp,sp);
      revd_line(11,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
      revd_linex(12,sp, A, c, t, i, v, a, t, e,sp, t, h, i, s,sp, c, h, e, a, t,sp, a, n, d,sp, t, r, y,sp);
      revd_linex(13,sp, y, o, u, r,sp, l, u, c, k,sp, a, g, a, i, n, s, t,sp, s, o, m, e,sp, R, E, A, L,sp);
      revd_line(14,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp, h, i, s, c, o, r, e, s,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   }               

   cpy(&oi_buffer[0], (void *)MEM_OAM, 256);
   revd_line(16,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);
   revd_linex(17,sp,sp,sp, P, r, e, s, s,sp, S, t, a, r, t,sp, t, o,sp, C, o, n, t, i, n, u, e,sp,sp,sp);
   revd_line(18,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp,sp);

#define rebrightness(amount) cpy((void *)&tpal[(31-amount)] , (void *)&pal_bg_mem[0], 128);


   while (RE_STATUS != RE_STATUS_DEAD) {
      if (RE_STATUS == RE_STATUS_FADEIN) {
         if (animationframe == 0) {
            REG_DISPCNT = 0 | VID_BG0;
            REG_BLDMOD = bit7 | bit6 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
            REG_COLEY  = 16;
         ...
         if (animationframe == 32) {
            REG_DISPCNT = 0 | VID_BG0 | VID_BG1 | VID_BG2 | VID_OBJ | VID_LINEAR;
            animationframe = 0;
            RE_STATUS = RE_STATUS_APPEAR;
            REG_IE |= INT_HBLANK;
            HBlanksituation = HBLANK_REVELWIPE;
            hfadeline = 0;
         }
      } else if (RE_STATUS == RE_STATUS_APPEAR) {
         VSync();
         key_poll();
         animationframe++;
         hfadeline = animationframe * 4;
         u8 brightness = 31-(animationframe>>2);
         rebrightness(brightness);
         if (animationframe == 62) {
            RE_STATUS = RE_STATUS_STEADY;  
            REG_IE &= ~INT_HBLANK;
            REG_COLEY = REG_COLEV = REG_BLDMOD = 0;
         }
      } else if (RE_STATUS == RE_STATUS_STEADY) {
         VSync();
         key_poll();
         if (key_hit(KEY_START) || key_hit(KEY_A) || key_hit(KEY_SELECT) || key_hit(KEY_B)) {
            RE_STATUS = RE_STATUS_FADEOUT;
            REG_BLDMOD = bit7 | bit6 | bit5 | bit4 | bit3 | bit2 | bit1 | bit0;
            REG_COLEY  = 0;                                                    
            animationframe = 32;
         }
      } else if (RE_STATUS == RE_STATUS_FADEOUT) {
         VSync();
         key_poll();
         animationframe--;
         REG_COLEY = 16 - (animationframe >> 1);
         AAS_MOD_SetVolume(animationframe << 3);
         if (animationframe == 0) {
            AAS_MOD_Stop();
            AAS_MOD_SetVolume(255);
            REG_DISPCNT = 0;
