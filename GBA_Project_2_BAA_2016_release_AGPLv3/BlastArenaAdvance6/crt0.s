...
@ - Modified by James Daniels/Apex Designs to give AAS interrupts higher priority in special
@   "AAS_MultipleInterrupts" mode
...
@   enable this option.
@
@ __AAS_MultipleInterrupts
@   Uncomment this line to allow multiple-interrupts-at-once
@   support, giving the AAS interrupt higher priority. If
@   you have several interrupts where one can occur while
@   another is being serviced then you need to enable this
@   option. Only for use with AAS! Added by James Daniels.

@ .equ __FastInterrupts, 1
@ .equ __SingleInterrupts, 1
@ .equ __MultipleInterrupts, 1
.equ __AAS_MultipleInterrupts, 1
...
@        strh    r3,  [r2]                @ set IE
  ...
  .ifdef __AAS_MultipleInterrupts
intr_main:
                                         @ Multiple interrupts support
        mov     r2, #0x4000000           @ REG_BASE
        ldr     r3, [r2,#0x200]!         @ r2 = IE : r3 = IF|IE
...
@ mode if __SwitchToUserStack is defined.
				...
				ldr     r12, =AAS_IntrTable
				adr 		r0, AAS_intr_start+8
				sub     r14, r14, r0
				mov     r14, r14, lsr #3
        ldr     r0, [r12, r14, lsl #2]
...
@ Switch to system mode if __SwitchToUserStack is defined.
  ...
  .endif
    ...
    .ALIGN
    .POOL               @ If you copy the intr_main routine, above, to internal
